...
test.beforeEach(async ({page}) => {
  ...
  test('T2. Show line item tab', async({ page })=>{
    //const data= await RuleEngineDataParser('./mock-data/ruleEngineMockData.csv', 'T1');
  ...
  test('T3. Hide Allocation tab from setting & verify', async({ page })=>{
    await page.click('div[role="button"] .v-list-item__title:has-text("Admin")');
    await page.getByRole('link', { name: 'Invoice Settings' }).click();
    await page.waitForTimeout(400);
    await page.locator('div[role="tab"]:has-text("Allocations")').click();
    await page.waitForTimeout(400);
    const section = page.locator('h5', { hasText: 'Start allocation tab' }).locator('..'); // Go to parent of the heading
    const radioGroup = section.locator('input[type="radio"][value="false"]'); // false = Off
   ...
   if (await unsavedDialog.isVisible()) {
    ...
    await page.waitForTimeout(10000);
    const tabGroup = page.locator('.v-slide-group__content');
    const lineItemTab = tabGroup.locator('div[role="tab"]', { hasText: 'Allocations' });
    await expect(lineItemTab).toHaveCount(0);
    await page.waitForTimeout(10000);
  });
  test.only('T4. Enable Allocation tab from invoice setting & verify', async({ page })=>{
    await page.click('div[role="button"] .v-list-item__title:has-text("Admin")');
    await page.getByRole('link', { name: 'Invoice Settings' }).click();
    await page.waitForTimeout(400);
    await page.locator('div[role="tab"]:has-text("Allocations")').click();
   ...
   if (await unsavedDialog.isVisible()) {
    ...
    await rows.nth(0).click();
    await page.waitForTimeout(10000);
    await expect(page.locator('div[role="tab"]', { hasText: 'Allocations' })).toHaveCount(1);
    await page.waitForTimeout(10000);
  });
